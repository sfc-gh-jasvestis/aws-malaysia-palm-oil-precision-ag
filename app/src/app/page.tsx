...
export default function HomePage() {
  ...
  const askAiTab = (
    <div className="h-[600px]">
      <AskAI title="Ask AI" sampleQuestions={['Which estates have yield below 18T/ha?', 'Show fertilizer ROI by soil type', 'What is the optimal replanting schedule?']} onSubmit={async (question, mode) => ({ answer: `[Demo Mode] Response to: "${question}" (${mode} mode). Connect to Snowflake for live data.`, sql: mode === 'sql' ? 'SELECT * FROM CURATED.SUMMARY LIMIT 10;' : undefined })} />
    </div>
  );
